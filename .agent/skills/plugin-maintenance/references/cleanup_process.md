...
This document explains the logic used by `sync_with_inventory.py` to manage plugin lifecycles in consuming repositories. It is invoked via the **[plugin-maintenance](../SKILL.md)** skill (Sync operation). The goal is to keep vendor plugins up-to-date while protecting project-specific customizations.
