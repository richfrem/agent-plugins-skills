...
If `sync_with_inventory.py` reports it cannot locate the vendor inventory file:
...
If `sync_with_inventory.py` reports a target directory (`.agent/`, `.gemini/`, etc.) does not exist:
- **Action**: Do NOT create the directory manually. Report to the user that the agent environment has not been initialized. Suggest running the `agent-bridge` skill from `plugin-mapper` to initialize the environment first.

## 4. Audit Script Unavailable
If `audit_structure.py` cannot be found or exits with a non-zero code:
