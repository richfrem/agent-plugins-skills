...
## Post-Merge Plugin Parity Checklist

After a plugin, skill, script, reference, agent, or plugin manifest change is merged into
`origin/main`, closeout must verify the complete published surface, not only the files named in
the pull request. This prevents a source change from leaving installers, documentation, or runtime
links stale.

For each affected plugin, record one of **updated**, **verified unchanged**, or **not applicable**
for every item below:

1. `plugins/<plugin>/plugin.yaml` — authoritative plugin inventory and version.
2. `plugins/<plugin>/.claude-plugin/plugin.json` — plugin-system metadata and description.
3. Root `.claude-plugin/marketplace.json` — marketplace description, version-facing metadata, and
   source path.
4. Root `README.md` and `architecture.md` — skill counts, plugin topology, and user-facing
   capability descriptions.
5. `symlinks.json` — all added, moved, or removed file-level links, managed only through
   `symlink_manager.py`.
6. `skills-lock.json` — regenerated by the installer; never hand-edited.
7. Installed runtime copies under `.agents/` — reinstall the affected plugin from local source,
   then verify the changed executable content is present.

Required closeout commands are:

```bash
git fetch origin main
git log --oneline origin/main -3
python3 plugins/plugin-manager/scripts/plugin_add.py plugins/<plugin> -y
python3 plugins/agent-scaffolders/scripts/audit.py --path plugins/<plugin>
...
If a manifest or document does not require a content change, explicitly report that it was
compared against the merged source and verified unchanged. Do not silently omit it. Normalize any
installer-generated worktree source entries before commit; installed runtime state is a validation
target, not a new source entry in `plugin-sources.json`.
