...
1. **Run inventory on self (security scanning is on by default):**
   ```bash
   python3 plugins/agent-plugin-analyzer/scripts/inventory_plugin.py --path plugins/agent-plugin-analyzer --format json
   ```

2. **Run scanner against test fixtures:**
   ```bash
   python3 plugins/agent-plugin-analyzer/scripts/inventory_plugin.py --path plugins/agent-plugin-analyzer/tests/gold-standard-plugin --format json
   python3 plugins/agent-plugin-analyzer/scripts/inventory_plugin.py --path plugins/agent-plugin-analyzer/tests/flawed-plugin --format json
   ```
