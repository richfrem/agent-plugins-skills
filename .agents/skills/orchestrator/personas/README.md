...
These subagents were cloned from the repo above. That borrowed the collection from another repo. 
...
These subagents are automatically available when placed in the `~/.claude/` directory. Claude Code will automatically detect and load them on startup. This will enable the CLAUDE.md to be available in global scope, may also conflict with other repository.
...
cp /path/to/CLAUDE.md /path/to/your/project/CLAUDE.md
```
