...
3.  **Perform Rigorous Audit**:
    ...
    *   **Self-Containment (Portability)**: Verify that **all scripts, references, and assets** a skill depends on exist inside the skill's own directory tree -- not at the parent plugin level. Plugin-level `scripts/` folders break `npx skills add` installation. For shared scripts across sibling skills, verify that relative **symlinks** are used (pointing from the secondary skill to the primary skill's copy). `npx skills add` dereferences symlinks into real files during installation.
