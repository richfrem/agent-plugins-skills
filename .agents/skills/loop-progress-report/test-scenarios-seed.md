...
| T49 | agentic-os-init | Interview skip: init does not repeat questions already answered in conversation | MEDIUM |
...
- **Acceptance criteria**: File `retrospectives/survey_[DATE]_[TIME]_skill-improvement-eval.md`
...
- **Target**: concurrent-agent-loop (SKILL.md v0.5.0)
...
### T10 — concurrent-agent-loop: Memory Persistence After Cycle
...
### T11 — concurrent-agent-loop: os-learning-loop Auto-Trigger at 3 Friction Events
...
### T12 — concurrent-agent-loop: Lock Contention Handling

- **Target**: concurrent-agent-loop (kernel.py)
...
### T18 — session-memory-manager: Baseline Promotion Decision Accuracy

- **Target**: session-memory-manager
...
### T19 — session-memory-manager: Semantic Deduplication Detection
...
### T20 — session-memory-manager: Test Registry Preservation
...
### T21 — session-memory-manager: Conflict Detection Before Write
...
### T22 — session-memory-manager: Memory Size Limit Enforcement
...
### T23 — session-memory-manager: Survey Completion
...
### T24 — session-memory-manager: Rollback on Rejected Write
...
### T25 — session-memory-manager: Falsified Hypothesis Entry
...
- **Target**: learning-loop
- **Hypothesis**: session-memory-manager is invoked during Phase VII and writes a session log
...
- **Known weaknesses**: Requires session-memory-manager to work correctly (dependency).
...
### T49 — agentic-os-init: Interview Skip for Already-Answered Questions

- **Target**: agentic-os-init
...
### T50 — agentic-os-init: Survey Completion After Onboarding
...
- **Recommended next test**: Review whether the improvement recommendation from T50
  leads to an update of the agentic-os-init SKILL.md — closing the flywheel.
...
then session-memory-manager and os-learning-loop (most impactful for flywheel),
then concurrent-agent-loop and dual-loop (integration tests).
