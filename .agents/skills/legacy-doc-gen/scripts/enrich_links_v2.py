#!/usr/bin/env python3
"""
[SKILL_ROOT]/scripts/enrich_links_v2.py (CLI)
=====================================
...
Usage Examples:
    python [SKILL_ROOT]/scripts/enrich_links_v2.py --help
...
ORPHANED_ROLES_PATH = PROJECT_ROOT / 'legacy-system' / 'reference-data' / 'orphaned_roles.json'
...
VALID_ROLE_PREFIXES = ('JAS_', 'JCS_', 'JRS_', 'RCC_', 'OCJ_', 'POS_', 'LEA_', 'JUS_')
...
def get_relative_path(from_file: Path, to_path: str) -> str | None:
    ...
    abs_target = PROJECT_ROOT / to_path
...
def create_link(obj_id: str, from_file: Path, include_source_links: bool = False) -> str | None:
    ...
    Args:
        obj_id: The object identifier (e.g., 'RCCE0147', 'RCC_CORR_ADMIN')
...
def enrich_content(content: str, from_file: Path) -> str:
    ...
    # Normalize file extensions in brackets: [RCCE0147.fmb] -> [RCCE0147]
    ...
    def link_artifact_tag(m):
        ...
        # Fallback: Try to use ID from filename (e.g. JASE0016-Overview.md -> JASE0016)
        if not obj_id:
    ...
    # Forms/Reports: XXXX0000 pattern (e.g., RCCE0147, JCSE0030)
    ...
    # e.g. Allow (JASE0016) but not [Link](JASE0016) or [JASE0016]
    ...
    # This catches items like JUSLIB, WEBUTIL, etc. that don't match specific patterns.
...
def main():
    ...
    if args.file:
        ...
        sys_dir = PROJECT_ROOT / 'legacy-system'
        dirs = [
            sys_dir / 'oracle-forms-overviews' / 'forms',
            sys_dir / 'oracle-forms-overviews' / 'reports',
            sys_dir / 'oracle-forms-overviews' / 'menus',
            sys_dir / 'oracle-forms-overviews' / 'libraries',
            sys_dir / 'business-workflows',
            sys_dir / 'business-rules',
            sys_dir / 'applications',
            sys_dir / 'oracle-database-overviews' / 'tables',
            sys_dir / 'oracle-database-overviews' / 'views',
            sys_dir / 'oracle-database-overviews' / 'packages',
            sys_dir / 'oracle-database-overviews' / 'procedures',
            sys_dir / 'oracle-database-overviews' / 'functions',
            sys_dir / 'oracle-database-overviews' / 'constraints',
            sys_dir / 'oracle-database-overviews' / 'indexes',
            sys_dir / 'oracle-database-overviews' / 'sequences',
            sys_dir / 'oracle-database-overviews' / 'types',
        ]
