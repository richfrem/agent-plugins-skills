...
import { itemProcessor } from '../../src/processors/elements/ItemProcessor.js';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/**
 * Test suite for the Item Processor
 * Tests the processing of Oracle Forms Item elements using all XML files 
 * found in test-data/Item/ directory
 */
describe('Item Processor', () => {
    const outputDir = path.join(__dirname, '../../test-output/elements/Item');
    ...
    // Helper function to process Item files
    const processItemFile = async (filename) => {
        const xmlPath = path.join(__dirname, '../../test-data/Item', filename);
        const xmlContent = fs.readFileSync(xmlPath, 'utf-8');
        const parsedXml = await parseTestXml(xmlContent);
        
        // Process each Item element
        if (Array.isArray(parsedXml.Item)) {
            for (const element of parsedXml.Item) {
                const processed = await itemProcessor.processRootElement(element);
                const output = [];
                await itemProcessor.formatResults(processed, () => {}, output);
                allOutput.push(...output);
            }
        } else if (parsedXml.Item) {
            const processed = await itemProcessor.processRootElement(parsedXml.Item);
            const output = [];
            await itemProcessor.formatResults(processed, () => {}, output);
            allOutput.push(...output);
        }
    ...
    const testFiles = fs.readdirSync(path.join(__dirname, '../../test-data/Item'))
    ...
    testFiles.forEach(file => {
        test(`processes ${file.name}`, async () => {
            ...
            await processItemFile(file.name);
