...
import { eventProcessor } from '../../src/processors/elements/EventProcessor.js';
 ...
 * Tests the processing of Oracle Forms Event elements using all XML files 
 * found in test-data/Event/ directory
 */
describe('Event Processor', () => {
    const outputDir = path.join(__dirname, '../../test-output/elements/Event');
    ...
    const processEventFile = async (filename) => {
        const xmlPath = path.join(__dirname, '../../test-data/Event', filename);
        const xmlContent = fs.readFileSync(xmlPath, 'utf-8');
        const parsedXml = await parseTestXml(xmlContent);
        
        // Process each Event element
        if (Array.isArray(parsedXml.Event)) {
            for (const element of parsedXml.Event) {
                const processed = await eventProcessor.processRootElement(element);
                const output = [];
                await eventProcessor.formatResults(processed, () => {}, output);
                allOutput.push(...output);
            }
        } else if (parsedXml.Event) {
            const processed = await eventProcessor.processRootElement(parsedXml.Event);
            const output = [];
            await eventProcessor.formatResults(processed, () => {}, output);
            allOutput.push(...output);
        }
    ...
    const testFiles = fs.readdirSync(path.join(__dirname, '../../test-data/Event'))
    ...
    testFiles.forEach(file => {
        test(`processes ${file.name}`, async () => {
            ...
            await processEventFile(file.name);
