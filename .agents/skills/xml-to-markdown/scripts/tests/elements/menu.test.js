...
import { menuProcessor } from '../../src/processors/elements/MenuProcessor.js';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/**
 * Test suite for the Menu Processor
 * Tests the processing of Oracle Forms Menu elements using all XML files 
 * found in test-data/Menu/ directory
 */
describe('Menu Processor', () => {
    const outputDir = path.join(__dirname, '../../test-output/elements/Menu');
    ...
    // Helper function to process Menu files
    const processMenuFile = async (filename) => {
        const xmlPath = path.join(__dirname, '../../test-data/Menu', filename);
        const xmlContent = fs.readFileSync(xmlPath, 'utf-8');
        const parsedXml = await parseTestXml(xmlContent);
        
        // Process each Menu element
        if (Array.isArray(parsedXml.Menu)) {
            for (const element of parsedXml.Menu) {
                const processed = await menuProcessor.processRootElement(element);
                const output = [];
                await menuProcessor.formatResults(processed, () => {}, output);
                allOutput.push(...output);
            }
        } else if (parsedXml.Menu) {
            const processed = await menuProcessor.processRootElement(parsedXml.Menu);
            const output = [];
            await menuProcessor.formatResults(processed, () => {}, output);
            allOutput.push(...output);
        }
    ...
    const testFiles = fs.readdirSync(path.join(__dirname, '../../test-data/Menu'))
    ...
    testFiles.forEach(file => {
        test(`processes ${file.name}`, async () => {
            ...
            await processMenuFile(file.name);
