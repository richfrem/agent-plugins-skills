---
description: Complete Database Sequence documentation workflow.
inputs: [SequenceName]
tier: 3
---
...
**Purpose:** Documentation workflow for Oracle Database Sequences. Calls `/investigate-db-sequence`.


---

> **SYSTEM ENFORCEMENT PROTOCOL**
> 1. **MANDATORY EXECUTION**: You MUST invoke the `/legacy-system-database_investigate-db-sequence` skill via the Skill tool.
...
```bash
/agent-orchestrator_plan "codify-db-sequence-[SequenceName]"
```
...
**What this does:**
1. Creates `.worktrees/###-codify-db-sequence-[SequenceName]-WP01/` directory
...
```bash
/legacy-system-database_investigate-db-sequence [SequenceName]
```

**What this does:**
1. Initializes context (`--direction downstream`, up to 3 levels)
2. Executes Sequence Mining
...
**Template:** `plugins/legacy system/assets/templates/db-sequence-template.md`
