---
description: Complete Database Constraint documentation workflow.
inputs: [ConstraintName]
tier: 3
---

# /codify-db-constraint

**Purpose:** Documentation workflow for Oracle Database Constraints. Calls `/investigate-db-constraint`.

---

> **SYSTEM ENFORCEMENT PROTOCOL**
> 1. **MANDATORY EXECUTION**: You MUST invoke the `/legacy-system-database_investigate-db-constraint` skill via the Skill tool.
...
```bash
/agent-orchestrator_plan "codify-db-constraint-[ConstraintName]"
```
...
**What this does:**
1. Creates `.worktrees/###-codify-db-constraint-[ConstraintName]-WP01/` directory
...
```bash
/legacy-system-database_investigate-db-constraint [ConstraintName]
```

**What this does:**
1. Initializes context (`--direction downstream`, up to 3 levels)
2. Executes Constraint Mining
...
**Template:** `plugins/legacy system/assets/templates/db-sops/sop-codify-db-constraint-template.md`
