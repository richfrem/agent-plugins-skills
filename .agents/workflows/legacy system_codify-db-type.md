---
description: Complete Database Type documentation workflow.
inputs: [TypeName]
tier: 3
---
...
**Purpose:** Documentation workflow for Oracle Database Types. Calls `/investigate-db-type`.


---

> **SYSTEM ENFORCEMENT PROTOCOL**
> 1. **MANDATORY EXECUTION**: You MUST invoke the `/legacy-system-database_investigate-db-type` skill via the Skill tool.
...
```bash
/agent-orchestrator_plan "codify-db-type-[TypeName]"
```
...
**What this does:**
1. Creates `.worktrees/###-codify-db-type-[TypeName]-WP01/` directory
...
```bash
/legacy-system-database_investigate-db-type [TypeName]
```

**What this does:**
1. Initializes context (`--direction downstream`, up to 3 levels)
2. Executes Type Mining
...
**Template:** `plugins/legacy system/assets/templates/db-type-template.md`
