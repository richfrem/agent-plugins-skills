...
| R3 | **Float precision makes baseline unreachable — CONFIRMED + FIXED** | `evaluate.py`, `results.tsv` | **Bug confirmed by live test:** `(1.0 * 0.7) + (0.7 * 0.3)` returns `0.9099999999999999` in Python float. The BASELINE row stores `"0.9100"` (4dp), which parses back as `0.91`. Since `0.9099999999999999 < 0.91`, every iteration DISCARDs even with zero change — the loop can never make progress from a perfect-routing skill. Fixed by changing comparison to `round(score, 4) >= round(baseline_score, 4)`. **Verify this fix doesn't open a rounding exploit:** does a score of 0.90945 (rounds to 0.9095) correctly DISCARD against a baseline of 0.9100? Does 0.90995 (rounds to 0.9100) pass when it should borderline-KEEP? |
...
3. **Float precision fix (R3):** The `round(score, 4)` comparison fixes the false-DISCARD bug. Does rounding to 4dp introduce a new exploit — can an agent achieve score 0.90995 (rounds to 0.9100) and pass a baseline of 0.9100 when the true improvement is negligible? Is 4dp the right granularity?
