---
concept: workflow-retrospective
source: plugin-code
source_file: spec-kitty-plugin/assets/templates/workflow-retrospective-template.md
wiki_root: /Users/richardfremmerlid/Projects/agent-plugins-skills/.wiki
generated_at: 2026-04-17T06:42:10.370521+00:00
cluster: user
content_hash: 4deb6be8d638d34d
---

# Workflow Retrospective

> *Summary pending — run /wiki-distill*

## Key Ideas

- *(Bullets pending — run /wiki-distill)*

## Details

# Workflow Retrospective

**Date**: {{DATE}}
**Workflow**: {{WORKFLOW}}
**Spec**: {{SPEC_ID}}

## Part A: User Feedback (MANDATORY)

> **Agent Instruction**: Ask the user the 4 questions below. Copy their answers here verbatim.

**1. What went well for you during this workflow?**
> [User Answer Here]

**2. What was frustrating or confusing?**
> [User Answer Here]

**3. Did I (the Agent) ignore any of your questions or feedback?**
> [User Answer Here]

**4. Do you have any suggestions for improvement?**
> [User Answer Here]

---

## Part B: Agent Self-Assessment

**1. Workflow Smoothness**
- [ ] **Smooth** (0-1 retries)
- [ ] **Bumpy** (2+ retries)
  - *If bumpy, explain why:*

**2. Tooling Gaps**
- [ ] CLI Tools failed or confused me
- [ ] Templates were missing sections
- [ ] Workflow steps were unclear

---

## Part C: Gap Analysis (Technical Debt)

> Did you identify non-critical issues or tech debt?

- [ ] **No**
- [ ] **Yes** (List below)
  1. [Task Name] (Priority)

---

## Part D: Immediate Improvement (Boy Scout Rule)

> Choose ONE action you performed *during* this retrospective to leave the codebase better.

- [ ] **Fixed Code**: Fixed a bug in `[file]`.
- [ ] **Fixed Docs**: Clarified `[file]`.
- [ ] **Created Task**: Logged `[Task ID]` for future work.
- [ ] **No Issues**: Everything was perfect.

**Action Details**:
> Describe what you did.


## See Also

- [[workflow-retrospective-template]]
- [[metrics-workflow-health-continuous-improvement]]
- [[learning-loop-retrospective-post-seal]]
- [[l4-chained-commands-workflow-graphs]]
- [[pattern-pre-execution-workflow-commitment-diagram]]
- [[pre-execution-workflow-commitment-diagram]]

## Raw Source

- **Source:** `plugin-code`
- **File:** `spec-kitty-plugin/assets/templates/workflow-retrospective-template.md`
- **Indexed:** 2026-04-17T06:42:10.370521+00:00
