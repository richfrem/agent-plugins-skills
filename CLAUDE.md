...
This is the **canonical upstream source** for a universal agent plugins and skills ecosystem. It publishes plugins consumed by Claude Code, GitHub Copilot, Gemini CLI, Windsurf, Cursor and other agent frameworks. All plugins live under `plugins/` and are installed into agent environments (`.claude/`, `.agents/`, `.github/`, etc.) via the bridge installer.

## Cross-Repo Coordination

This repo works in tandem with a LAB session in `spec-kitty-improvements/`. Both share a message pad:
...
- Entry types: `EVAL-RESULT`, `FIX-APPLIED`, `QUESTION`, `REQUEST`, `INFO`, `BLOCKED`

This session's role: **UPSTREAM** — apply validated fixes to canonical plugin source, then post a `FIX-APPLIED` entry so LAB knows to sync and re-run evals.
...
```bash
# Install all plugins into local agent environments (.claude/, .agents/, etc.)
...
# Install from an alternate plugins directory
python plugins/plugin-manager/scripts/install_all_plugins.py --plugins-dir path/to/plugins
```

### Audit & Validate

```bash
# Audit a plugin for ecosystem standard compliance
python plugins/agent-plugin-analyzer/scripts/audit.py --path plugins/<plugin-name>

# Run trigger evals for a skill description
python plugins/agent-plugin-analyzer/scripts/run_eval.py \
  --eval-set plugins/<plugin>/skills/<skill>/evals/evals.json \
  --skill-path plugins/<plugin>/skills/<skill>

# Run the description improvement loop (eval + improve iteratively)
python plugins/agent-plugin-analyzer/scripts/run_loop.py \
  --eval-set evals.json --skill-path plugins/<plugin>/skills/<skill>
```
...
python plugins/agent-plugin-analyzer/scripts/check_for_broken_symlinks.py

# Validate plugin boundaries (no cross-plugin imports)
python plugins/agent-plugin-analyzer/scripts/check_plugin_boundaries.py
```
...
Each plugin under `plugins/<plugin-name>/` follows this layout:
...
plugins/<plugin-name>/
  ...
  skills/
    <skill-name>/
      ...
      evals/evals.json     # Trigger evaluation set
      scripts/             # Python scripts executed by the skill
      requirements.txt     # Per-skill dependencies (pip-compile managed)
  agents/                  # Sub-agent .md definitions (optional)
  commands/                # Slash-command .md definitions (optional)
  hooks/                   # Hook definitions (optional)
  assets/                  # Static resources
```

### Deployment: `.agents/` Central Hub

The bridge installer copies/symlinks plugin skills into `.agents/skills/<plugin-name>-<skill-name>/`. Agent-specific environments (`.claude/commands/`, `.github/copilot-instructions.md`, etc.) then link from `.agents/`. The `skills-lock.json` at the repo root tracks installed hashes.

### CLAUDE.md Auto-Generated Section

The `<!-- BEGIN RULES FROM PLUGIN: ... -->` blocks below are injected by `plugins/plugin-mapper/`. Do not manually edit those blocks — re-run the plugin mapper to regenerate them.

---
<!-- AUTO-GENERATED PLUGIN RULES BELOW — managed by plugin-mapper, do not manually edit -->
