...
If the primary scanning engine (`./../scripts/find_json_duplicates.py`) exits with an error status, execute the following triage steps exactly in order:

## 1. Syntax Error Rejection (Exit Code 2)
If `./../scripts/find_json_duplicates.py` exits with `2`, the file is not valid JSON. This usually means missing or trailing commas, unescaped quotes, or mismatched braces.
...
If `./../scripts/find_json_duplicates.py` exits with `1`, duplicate keys definitively exist in the file.
