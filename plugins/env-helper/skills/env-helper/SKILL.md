...
- **Resolver Engine**: `./env_helper.py`

## Usage Examples

```bash
# Resolve a single key (most common)
python3 ./env_helper.py --key HF_TOKEN

# Dump all known constants as JSON
python3 ./env_helper.py --all

# Get the full HuggingFace upload config block
python3 ./env_helper.py --hf-config
```
...
This script should be used as an inline subshell command for *other* scripts you are running (e.g. `export HF_TOKEN=$(python3 ./env_helper.py --key HF_TOKEN)`).
...
## Next Actions
If the `env_helper.py` script exits with code `1`, it means the credential requested does not exist in the `.env` file or process environment, and it has no default. Consult the `./fallback-tree.md` immediately.
