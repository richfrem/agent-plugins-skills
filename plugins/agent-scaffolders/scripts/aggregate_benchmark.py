...
def _resolve_search_dir(benchmark_dir: Path) -> Path | None:
    """Resolve which directory to search for eval-* subdirectories."""
...
def _load_single_run(run_dir: Path, eval_id: int, run_number: int) -> dict | None:
    """Load and parse a single run's grading.json, returning a result dict or None."""
    ...
    for exp in raw_expectations:
        if "text" not in exp or "passed" not in exp:
            print(f"Warning: expectation in {grading_file} missing required fields: {exp}")
    result["expectations"] = raw_expectations
    return result


def _collect_run_notes(grading: dict) -> list:
    """Collect notes from grading's user_notes_summary block."""
    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                result = _load_single_run(run_dir, eval_id, run_number)
                if result is not None:
                    grading_file = run_dir / "grading.json"
                    try:
                        with open(grading_file) as f:
                            grading = json.load(f)
                        result["notes"] = _collect_run_notes(grading)
                    except (json.JSONDecodeError, OSError):
                        result["notes"] = []
...
def _build_runs_array(results: dict) -> list:
    """Build the runs array for benchmark.json from aggregated results."""
    runs = []
...
def _build_metadata(results: dict, skill_name: str, skill_path: str) -> dict:
...
def generate_benchmark(benchmark_dir: Path, skill_name: str = "", skill_path: str = "") -> dict:
    ...
    return {
        "metadata": _build_metadata(results, skill_name, skill_path),
        "runs": _build_runs_array(results),
        "run_summary": run_summary,
        "notes": []  # To be filled by analyzer
    }


def _format_summary_table(run_summary: dict, configs: list) -> list[str]:
    """Build the markdown summary table rows for two configs."""
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines.extend(_format_summary_table(run_summary, configs))
...
def _write_outputs(benchmark: dict, benchmark_dir: Path, output: Path | None) -> tuple[Path, Path]:
    """Write benchmark.json and benchmark.md, return their paths."""
    output_json = output or (benchmark_dir / "benchmark.json")
...
def _print_summary(benchmark: dict) -> None:
...
def main() -> None:
    ...
    _write_outputs(benchmark, args.benchmark_dir, args.output)
