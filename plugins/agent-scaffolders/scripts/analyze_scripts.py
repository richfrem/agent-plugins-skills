...
def _print_single_use_analysis(uses_list: List[str], physical_locations: List[Path], physically_exists: bool, plugin_path: Path) -> None:
    """Print analysis details for a script used by exactly one skill."""
...
def _print_multi_use_analysis(
    uses: Set[str], exists_in_root: bool, physically_exists: bool,
    physical_locations: List[Path], symlink_locations: List[Path], plugin_path: Path
) -> None:
    """Print analysis details for a script used by multiple skills."""
...
def _print_script_analysis(
    ...
    if len(uses) == 1:
        _print_single_use_analysis(uses_list, physical_locations, physically_exists, plugin_path)
    elif len(uses) > 1:
        _print_multi_use_analysis(uses, exists_in_root, physically_exists, physical_locations, symlink_locations, plugin_path)

def _analyze_single_plugin(plugin_path: Path, whitelist: Dict[str, Any]):
    """Run the full analysis for one plugin, print its section, and return
    (plugin_name, missing_entries) if it has unresolved scripts, else None."""
    ...
    if not filtered_missing_scripts:
        return None
    ...
    for s in sorted(filtered_missing_scripts):
        ...
        entries.append((s, uses))
        print(f"      - {s} (Referenced by: {', '.join(uses)})")

    return plugin_name, entries
...
def analyze_plugins(base_dir: str) -> None:
    ...
    for plugin_path in plugins_dir.iterdir():
        ...
        result = _analyze_single_plugin(plugin_path, whitelist)
        if result:
            plugin_name, entries = result
            global_missing_summary[plugin_name] = entries
