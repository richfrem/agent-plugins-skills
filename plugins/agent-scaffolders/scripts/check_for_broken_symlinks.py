...
def save_outputs(inventory_list: List[Dict[str, Any]], report_lines: List[str], fix_commands: List[str]) -> None:
    ...
    """
    inventory_path = os.path.join(PLUGINS_DIR, "agent-scaffolders/references/plugin_files_and_symlinks_inventory.json")
    os.makedirs(os.path.dirname(inventory_path), exist_ok=True)
    with open(inventory_path, "w") as f:
        json.dump(inventory_list, f, indent=2)

    report_path = os.path.join(PLUGINS_DIR, "agent-scaffolders/references/broken_symlinks_repair_report.md")
    with open(report_path, "w") as f:
        f.write("\n".join(report_lines))

    fix_script_path = os.path.join(PLUGINS_DIR, "agent-scaffolders/scripts/apply_symlink_repairs.sh")
