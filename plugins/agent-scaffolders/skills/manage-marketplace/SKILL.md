...
6.  **Plugin Manifest Schema Requirements (MANDATORY)**:
    - Every plugin's `.claude-plugin/plugin.json` **MUST** adhere to the clean, minimal metadata schema:
      ```json
      {
        "name": "my-plugin",
        "version": "0.1.0",
      ...
      ```
    - **Prohibited fields**: `.claude-plugin/plugin.json` must **NEVER** contain `skills`, `agents`, `hooks`, or `commands` arrays (these are auto-discovered by Claude Code and rejected by validators).
