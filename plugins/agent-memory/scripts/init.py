...
def _get_default_profile_settings(existing_profile: dict) -> dict:
    """Return the default configuration dictionary merged with the existing settings."""
...
def _scaffold_default_manifest(manifest_path: Path) -> None:
    """Scaffold a default empty manifest if it doesn't already exist."""
...
def configure_profile() -> None:
    ...
    existing_profile = profiles_data.get("profiles", {}).get(KNOWLEDGE_PROFILE, {})
    
    print("[ARCH] Defaulting to In-Process Persistence (Direct disk access).")
    profile = _get_default_profile_settings(existing_profile)
    ...
    _scaffold_default_manifest(manifest_path)
