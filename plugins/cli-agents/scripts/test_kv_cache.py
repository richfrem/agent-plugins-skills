...
Key Input Dependencies:
    - kv_cache_orchestrator.py module (in the same directory)
    - Temporary directories (tempfile.mkdtemp) for cache storage fixtures
"""
...
def _make_cache(tmpdir, budget_bytes=4 * 1024 * 1024 * 1024, quant_config=None):
    """Build a KVCacheOrchestrator pointed at tmpdir with the given budget/quant config."""
...
def _mock_urlopen_ok():
    """Build a mock urlopen response with HTTP status 200."""
    resp = MagicMock()
    resp.status = 200
...
class TestCacheKeyGeneration(unittest.TestCase):
    ...
    def setUp(self):
        """Create a temporary cache directory and orchestrator instance."""
        self.tmpdir = tempfile.mkdtemp()
        self.cache = _make_cache(self.tmpdir)

    def test_same_system_prompt_same_key(self):
        """Verify identical system prompts produce identical cache keys."""
        msgs = [{"role": "system", "content": "You are a helpful assistant."}]
        self.assertEqual(self.cache.cache_key(msgs), self.cache.cache_key(msgs))

    def test_key_is_sha256_hex(self):
        """Verify the cache key is a 64-char lowercase hex SHA-256 digest."""
    ...
    def test_user_messages_excluded_from_key(self):
        """Verify varying user turns don't change the cache key (only system content counts)."""
    ...
    def test_empty_system_prompt_is_stable(self):
        """Verify messages with no system prompt produce a stable (repeatable) cache key."""
        msgs = [{"role": "user", "content": "hello"}]
        self.assertEqual(self.cache.cache_key(msgs), self.cache.cache_key(msgs))

    def test_key_matches_manual_sha256(self):
        """Verify the cache key equals a manually computed SHA-256 of the system content."""
    ...
    def test_multiple_system_messages_joined(self):
        """Verify multiple system messages are newline-joined before hashing."""
        msgs = [
    ...
    def test_content_as_parts_array_produces_stable_key(self):
        """Verify OpenAI multimodal parts-array content produces a stable, valid key."""
    ...
    def test_content_as_parts_and_string_same_text_same_key(self):
        """Verify parts-array and plain-string content with the same text produce the same key."""
    ...
    def test_content_as_parts_array_no_text_type_gives_empty_string(self):
        """Verify parts with no 'text' type (e.g. image-only) hash as empty system content."""
...
class TestCacheHitSkipsPrefill(unittest.TestCase):
    ...
    def _plant_bin(self, key):
        """Write a fake .bin slot-state file for the given key."""
    ...
    def test_check_cache_true_when_bin_exists(self):
        """Verify check_cache returns True when a .bin file exists for the key."""
    ...
    def test_check_cache_false_when_no_bin(self):
        """Verify check_cache returns False when no .bin file exists for the key."""
    ...
    def test_restore_calls_llama_server_restore_endpoint(self):
        """Verify restore_slot POSTs to the /slots/{slot}/restore endpoint with the correct filename."""
    ...
    def test_restore_increments_hit_count(self):
        """Verify a successful restore increments the metadata's hit counter."""
    ...
    def test_restore_returns_false_on_server_error(self):
        """Verify restore_slot returns False when the server call raises an exception."""
...
class TestCacheMissSavesAfterResponse(unittest.TestCase):
    ...
    def test_save_calls_llama_server_save_endpoint(self):
        """Verify save_slot POSTs to the /slots/{slot}/save endpoint with the correct filename."""
        key = self.cache.cache_key([{"role": "system", "content": "brand new prompt"}])
        self.assertFalse(self.cache.check_cache(key))

        def fake_urlopen(req, timeout=None):
            """Simulate the server writing the .bin file and return a 200 response."""
    ...
    def test_save_writes_metadata_sidecar(self):
        """Verify a successful save writes a complete metadata sidecar JSON file."""
    ...
    def test_save_returns_false_on_server_error(self):
        """Verify save_slot returns False and leaves no cache entry when the server call fails."""
...
class TestDiskBudgetEviction(unittest.TestCase):
    ...
    def setUp(self):
        """Create a temporary cache directory with a 1 MiB budget orchestrator."""
        self.tmpdir = tempfile.mkdtemp()
    ...
    def _plant_entry(self, label, hits, age_hours, tokens=1000):
        """Write a fake 500 KiB .bin file and matching metadata sidecar for a cache entry."""
    ...
    def test_oldest_least_hit_entry_evicted_first(self):
        """Verify the oldest, least-hit entry is evicted first when over budget."""
    ...
    def test_no_eviction_within_budget(self):
        """Verify no eviction occurs when total cache size is within budget."""
    ...
    def test_both_bin_and_json_removed_on_eviction(self):
        """Verify both the .bin file and its .json sidecar are removed together on eviction."""
    ...
    def test_high_token_entry_outscores_low_token_entry(self):
        """Verify an entry with more tokens gets a higher eviction score (kept longer)."""
        # Same hits, same file_size, same age — more tokens = higher eviction score = kept longer
        def _meta(tokens):
            """Build a metadata dict with fixed hits/size/age and the given token count."""
            return {
                "reason": "cold",
                "hits": 1,
...
class TestDifferentPromptsDifferentKeys(unittest.TestCase):
    ...
    def test_claude_copilot_delegation_get_separate_keys(self):
        """Verify distinct CLI system prompts (Claude/Copilot/delegation) produce distinct keys."""
    ...
    def test_whitespace_difference_produces_different_key(self):
        """Verify a trailing whitespace difference in system content changes the cache key."""
    ...
    def test_slot_parameter_respected(self):
        """Verify each orchestrator instance's configured slot number is used in the restore URL."""
...
class TestQuantValidation(unittest.TestCase):
    ...
    def _plant_meta(self, key, quant_config, tokens=500):
        """Write a metadata sidecar with the given quant_config and token count."""
        meta = {
            "key": key,
            "reason": "cold",
            "hits": 2,
    ...
    def test_restore_rejects_mismatched_quant(self):
        """Verify restore_slot rejects (no API call) when saved quant config differs from current."""
    ...
    def test_restore_accepts_matching_quant(self):
        """Verify restore_slot proceeds to the API call when quant config matches."""
    ...
    def test_restore_accepts_entry_with_no_quant_recorded(self):
        """Verify legacy entries without a quant_config field are not rejected."""
    ...
    def test_save_records_quant_config_in_sidecar(self):
        """Verify save_slot records the current quant_config and token count in the sidecar."""
        quant = {"ctk": "q8_0", "ctv": "q8_0"}
        cache = _make_cache(self.tmpdir, quant_config=quant)
...
class TestSlotLock(unittest.TestCase):
    ...
    def test_slot_lock_serializes_concurrent_slot_operations(self):
        """Verify restore_slot blocks until a concurrent save_slot releases the slot lock."""
        ...
        def recording_urlopen(req, timeout=None):
            """Record save/restore call ordering, blocking mid-save until signaled to finish."""
        ...
        def fake_save_urlopen(req, timeout=None):
            """Simulate the server writing the .bin file, then delegate to recording_urlopen."""
            with open(os.path.join(self.tmpdir, f"{key}.bin"), "wb") as f:
                f.write(b"saved")
...
class TestMaxTokensGuard(unittest.TestCase):
    ...
    def test_save_skips_oversized_entry(self):
        """Verify save_slot returns False (no API call) when tokens exceeds max_tokens_per_entry."""
    ...
    def test_save_proceeds_within_limit(self):
        """Verify save_slot proceeds normally when tokens is within max_tokens_per_entry."""
        cache = KVCacheOrchestrator(
            cache_dir=self.tmpdir,
            llama_base_url="http://localhost:8089",
            max_tokens_per_entry=4000,
