...
| `scripts/query.py` | CLI semantic search |
| `scripts/cleanup.py` | Remove deleted files from DB |

## Troubleshooting

- **Missing langchain.storage**: install `langchain-classic` — it contains the legacy storage components.
- **Profiles Path Error**: check that `PROJECT_ROOT` resolves 6 levels up (`parents[5]`) to the `.agent/` directory.
- **Port Conflict**: plugin uses `8110` by default (not Chroma's default `8000`).
- **In-Process vs HTTP**: if multiple processes need the same index concurrently, use HTTP Server mode.
