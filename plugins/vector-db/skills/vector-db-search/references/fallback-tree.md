...
If the primary database wrappers (`./../scripts/query.py`, `ingest.py`) fail, execute the following triage steps exactly in order:
...
If `./../scripts/query.py` or `ingest.py` crash stating the requested `--profile` name does not exist in `.agent/learning/vector_profiles.json`:
