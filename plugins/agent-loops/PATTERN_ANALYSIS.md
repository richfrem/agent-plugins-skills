...
This guide maps the generic Agent-Loops skills to standard industry terminology (e.g., Google ADK patterns) and provides a comparative reference for when and how to use them.
...
An outer agent/manager defines a strict strategy packet and hands it to an inner agent for execution. The outer agent acts as a controller that loops and verifies the output before continuing.
...
### When to Use
Use as a Strict Delegation Pattern where a clear strategy or specification can be defined upfront. The inner agent acts exclusively as an execution worker, completely decoupled from the overarching architecture.
