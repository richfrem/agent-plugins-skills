...
Key Input Dependencies:
    - file_inventory.json (from Step 1)
    - broken_links.json (from Step 3, optional — enables targeted fixing)
"""
import os
import json
import re
...
def _replace_link_match(match: re.Match, file_path: str, inventory: Dict[str, List[str]],
                         root_dir: str, dry_run: bool, fix_count: list) -> str:
    """Attempt to fix one broken link regex match; increments fix_count[0] on success."""
    label = match.group(1)
...
def _process_lines_skipping_code_blocks(content: str, link_pattern: "re.Pattern", replace_fn) -> str:
    """Apply replace_fn to each line's links, skipping fenced (``` or ~~~) code blocks."""
    in_code_block = False
    ...
    for line in lines:
        ...
        if in_code_block:
            ...
            processed_lines.append(link_pattern.sub(replace_fn, line))
...
def fix_links_in_file(
    ...
    fix_count = [0]
    replace_fn = lambda match: _replace_link_match(match, file_path, inventory, root_dir, dry_run, fix_count)
    new_content = _process_lines_skipping_code_blocks(content, link_pattern, replace_fn)
...
def _discover_files_to_fix(args, root_dir: str) -> tuple:
    """Determine which files to fix: targeted from broken_links.json, or a full repo walk.

    Returns (files_to_fix, broken_list, has_broken_links_json).
    """
    ...
    return files_to_fix, broken_list, has_broken_links_json


def main() -> None:
    """Parse CLI arguments, auto-fix unique link matches, and write remaining_broken_links.json."""
    ...
    files_to_fix, broken_list, has_broken_links_json = _discover_files_to_fix(args, root_dir)
