...
After Update/Init, you MUST propagate the new configuration to the agent ecosystem:

**A. Sync Workflows (Windsurf -> Plugin System)**
```bash
python3 plugins/spec-kitty/skills/spec-kitty-agent/scripts/sync_workflows.py
```

**B. Sync Rules & Context (Kernel Bridge)**
```bash
python3 plugins/spec-kitty/skills/spec-kitty-agent/scripts/speckit_system_bridge.py
```

**C. Deploy to Agents (Plugin Manager Handoff)**
Finally, invoke the Plugin Manager to deploy the synchronized artifacts:
```bash
python3 plugins/plugin-manager/skills/plugin-manager/scripts/install_all_plugins.py
```
