...
- **If the file EXISTS:**
  ...
  - If status is `In Progress` or `TBD`:
    ...
    - If the tag is PRESENT:
      ...
      - If `authorized_skill` matches "exploration-handoff" AND `phase_number` matches the dashboard phase:
...
1. **work-intake / Control Plane** → produce `exploration/DIAGNOSTIC_BRIEF.md` and transition task to `INTERVIEW` in `context/control_plane.db`
