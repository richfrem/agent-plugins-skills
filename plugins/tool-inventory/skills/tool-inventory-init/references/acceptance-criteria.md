...
- **Validation**: `.agent/learning/rlm_tools_manifest.json` exists and includes the `tools/` and `plugins/` globs for recursive searching, explicitly excluding `node_modules` and `.venv`.
