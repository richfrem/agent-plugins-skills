...
3.  **Perform Rigorous Audit**:
    ...
    *   **Self-Containment (Portability)**: Verify that **all scripts, references, and assets** a skill depends on exist inside the skill's own directory tree -- not at the parent plugin level. Plugin-level `scripts/` folders break `npx skills add` installation, which copies only the skill folder. If multiple skills share utility code, it must be duplicated into each skill's `scripts/` directory.
