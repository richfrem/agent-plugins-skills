...
### 1. Planning Status Assessment
Perform specification and task readiness assessment directly via native file inspection before delegating. Ensure the task has clear acceptance criteria, atomic scope, and verifiable tests.
...
When an architecture design, security assessment, or plan requires adversarial evaluation, delegate to the `red-team-review` skill to compile context packets and manage review rounds until approved.
