...
def main() -> None:
    ...
    if args.wpm < 100 or args.wpm > 1000:
        print(f"Error: WPM must be between 100 and 1000. Got: {args.wpm}", file=sys.stderr)
        sys.exit(1)

    input_path = Path(args.input)
    if not input_path.exists():
        print(f"Error: Word list not found: {input_path}", file=sys.stderr)
        sys.exit(1)

    tokens = json.loads(input_path.read_text(encoding="utf-8"))
    stream = generate_stream(tokens, args.wpm)

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(stream, ensure_ascii=False, indent=2), encoding="utf-8")

    total_ms = sum(t["delay_ms"] for t in stream)
    minutes = total_ms / 60000
    print(f"Generated {len(stream)} tokens at {args.wpm} WPM")
    print(f"Estimated reading time: {minutes:.1f} minutes")
