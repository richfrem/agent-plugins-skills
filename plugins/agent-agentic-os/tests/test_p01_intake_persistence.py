...
def test_source_candidate_requires_authorized_source_and_human_confirmation(tmp_path):
    control_plane = _control_plane_with_task(tmp_path)
    with pytest.raises(PersistenceInvariantViolation, match="authorized source"):
        control_plane.record_source_assisted_answer_candidate(
            "p01-task", "interview", "round-1", "scope", "P01 only", "docs/brief.md",
    ...
    with pytest.raises(PersistenceInvariantViolation, match="actor='human'"):
        control_plane.confirm_source_assisted_answer_candidate(candidate_id, actor="agent")
