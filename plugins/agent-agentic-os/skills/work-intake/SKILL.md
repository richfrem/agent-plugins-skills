...
2a. **Hard rule -- skips are the human's decision:** Never run `record-review-skip` (or any `--actor human` / `--human-confirmed` command) on the human's behalf, and never quote a chat message as the human's confirmation. If the human declines a review, they record it through the interactive command (`--interactive`); you present the command and wait.
