...
description: >
  ...
  (3) refactor to close loopholes identified by skill-improvement-eval.
  Always run a RED scenario BEFORE writing a new skill -- never skip the baseline.
  Integrates with skill-improvement-eval as the objective eval gate.
...
**Integrated with**:
- `skill-improvement-eval` -- runs `eval_runner.py` as the GREEN verification step
...
| GREEN phase | Write the skill. Run `skill-improvement-eval`. KEEP only if score >= baseline. |
...
## Phase 3: GREEN Verification -- skill-improvement-eval
...
4. **Run skill-improvement-eval** -- KEEP gate is mandatory before any Write.
...
- [skill-improvement-eval](../skill-improvement-eval/SKILL.md) -- eval_runner.py, KEEP/DISCARD logic
