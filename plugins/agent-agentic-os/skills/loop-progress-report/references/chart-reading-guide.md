...
Reference for interpreting `loop-progress-report` output charts and text summaries.
...
`concurrent-agent-loop` cycle must run before the chart has meaningful data.
...
See `../../references/improvement-ledger-spec.md` for the three-section ledger format
